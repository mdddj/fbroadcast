...
class FBroadcast {
  ...
  List _getReceivers(BuildContext context) {
...
//          _stickyMap.remove(key);
  ...
  /// [key]-message type
  /// [context] - context.
  void remove(FResultCallback receiver, {String key = '', BuildContext? context}) {
    if (!_textIsEmpty(key)) {
  ...
  Future<bool> _unregisterAsync(BuildContext context) async {
...
abstract class FResultCallback<T> {
...
class _Notifier<T> {
  ...
  void notifyListeners() {
    final List<FResultCallback<T>> localListeners =
        List<FResultCallback<T>>.from(_listeners);
    for (final FResultCallback<T> listener in localListeners) {
      try {
        if (_listeners.contains(listener)) listener.run(value, callback);
      } catch (exception) {}
    }
    callback = null;
  }
